...
$(document).ready(function () {
    ...
    $.post("../../controller/ctrMantUser.php?op=get_datos_usuario", { usu_id: usu_id, contador : 1 },
    ...
    $("#btnEditarInfo").click(function () {
        ...
        function (data, textStatus, jqXHR) {
            data=JSON.parse(data);
                $('#modal_usu_celular').val(data.usu_celular),
                $('#modal_usu_direccion').val(data.usu_direccion) 
        },
    );

        function recuperarDatosAjax() { //recuperar datos para actualizar desde la vista config.php
            let registro = {
                usu_id: $('#usu_id').val(),
        ...
        $("#BotonModificar").click(function () {
            ...
            let registro = recuperarDatosAjax();
            update_usuario(registro);
            $("#modalConfigUsu").modal("hide");
        });

        function update_usuario(registro) { //actualizar desde la vista config.php
            $.ajax({
                type: "POST",
    ...
    $.post("../../controller/ctrCertificaciones.php?combo_certificacion=get_certificacion_x_usu", { usu_id: usu_id },
        function (data, textStatus, jqXHR) {
            // let htmlTemplate = '';
            // data.forEach(elem => {
            //     htmlTemplate += `
    ...
    $("#btnAgregarCertificacion").click(function () {  
        ...
        $.post("../../controller/ctrCertificaciones.php?combo_certificacion=combo_certificacion",
            function (data, textStatus, jqXHR) {
    ...
    //Inicio de servicio habilidades
        ...
        //Inicio Certificaciones general
        $.post("../../controller/ctrHabilidad.php?combo_habilidad=get_habilidad_x_usu", { usu_id: usu_id },
        function (data, textStatus, jqXHR) {
