...
$(document).ready(function () {
  ...
  tabla = $("#tabla_detalle").dataTable({
    ...
    $.post("../../controller/ctrGraficos.php?op_grafico=get_total_max_ts_evento_x_cada_servicio", function (data, textStatus, jqXHR) {
        prod_nombre=data.map((elem)=> [elem.prod_nombre]);
        total=data.map((elem)=> [elem.total]);
...
function btnVerDetalleTarea(event_id){
    ...
    $.post("../../controller/ctrGraficos.php?op_grafico=get_datos_tareas_datatables_detalle_cliente", {event_id:event_id},
        function (data, textStatus, jqXHR) {
            document.getElementById("nom_cliente_datatables_detalle_tareas_finalizadas").innerText=data[0].tick_titulo
        },
        "json"
    );
  ...
  tabla = $("#datatables_detalle_tareas_finalizadas").dataTable({
    ...
    "language": {
        ...
        "sInfo": "Mostrando un total de _TOTAL_ actividades",
        "sInfoEmpty": "Mostrando un total de 0 actividades",
